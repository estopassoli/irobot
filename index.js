...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Por favor aguarde um minuto estou processando seu pedido.',
				success: 'Tudo certo por aqui.',
				error: {
					stick: 'Erro ao converter figurinha, tente novamente',
					Iv: 'Link inválido.'
				},
				only: {
					group: 'Este comando só pode ser usado em grupos!',
					ownerG: 'Este comando só pode ser usado pelo dono do bot! ',
			...
			const ownerNumber = ["5513996597134"] // replace this with your number
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`Falha ao converter ${tipe} para figurinha, certifique-se que seja uma imagem ou um gif.`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
				...
				case 'listadmin':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('Marque o sticker que você quer transformar em imagem')
					reply(mess.wait)
